...
def _persist_entities(entities: Iterable[Entity]) -> None:
    seen: set[str] = set()
...
def run_cluster_operation(*, clusters_json: str | None = None) -> List[ClusterResult]:
    entities = db.load_entities()
    works = [e for e in entities if _is_work(e)]
    updated_works, clusters = cluster_works_by_title_responsibilities(works, entities)

    _persist_entities(updated_works)
...
def run_cluster_with_expression_operation(
    *,
    ...
    _persist_entities(updated_expressions)
