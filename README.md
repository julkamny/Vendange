...
- Expression clusters now render anchors coming from any work inside the clustered family (not just the anchor work); expressions that are clustered elsewhere vanish from the list instead of leaving empty groups, and the empty "no clustered expressions" placeholder has been removed to reduce noise.
