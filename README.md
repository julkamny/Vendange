...
- When a SPARQL filter is active, the banner now offers *Next/Previous* buttons (and shortcuts `Ctrl+Alt+↓` / `Ctrl+Alt+↑`) to jump through matching works or agents; the virtualized list scrolls and highlights the target row even when it was off-screen.
